...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (user.mfa_enabled) {
      const ok = user.mfa_secret ? verifyTOTP(String(user.mfa_secret), String(mfaCode || '')) : false
      if (!ok) {
    ...
    await payload.update({ collection: 'users', id: user.id, data: { login_attempts: 0, locked_until: null } })
    ...
    if (userDoc) {
      ...
      await payload.update({ collection: 'users', id: userDoc.id, data: { login_attempts: attempts, locked_until: lock } })
    }
